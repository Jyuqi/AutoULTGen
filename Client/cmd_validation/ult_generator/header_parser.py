...
class HeaderParser(object):
    ...
    def __init__(self, name, path):
        ...
        self.basic_type = {'int', 'bool', 'dword', 'uint8_t', 'uint16_t', 'uint32_t', 'uint64_t', 'size_t','char'}
    ...
    def get_datastructure(line):
        ...
        #3: 'stuct xxx : public xxx'
            ...
            super_structure_name = item_list[-1]   # multiple super structure name not considered yet
    ...
    def parse_basictype(line):
        ...
        if len(item_list) == 2:
            ...
            if line.find(':') != -1:
                type = item_list[0]
                name = item_list[1]
                address = line_clr[line_clr.find(':')+1 : ].strip()
        ...
        return type, name, address, prefix, value
    ...
    def parse_file_info(self):
        ...
        for index, line_clr in enumerate(new_lines):
            ...
            if any([line_clr.startswith(item)  for item in self.basic_type]):
                self.type , self.name , self.address , self.prefix, self.value = self.parse_basictype(line_clr)
                new_basic_group = SubElement(current_group, self.type, name=self.name)
                if self.address:
            ...
            if line_clr.find('_CMD') != -1:
                if current_group.tag == 'struct' and current_group.attrib['name']: 
                    if current_group.attrib['name'].endswith('_CMD'):
                    
                        #1: constructor xxx_CMD{xxx_CMD();}
                        constructor = current_group.attrib['name']+'()'
                        if line_clr.startswith(constructor):
                            constructor_group = SubElement(current_group, 'constructor')
                            text = line_clr.strip().strip(';').strip()
                            constructor_group.set('text', text)
                            continue
